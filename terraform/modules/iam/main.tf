...
locals {
  current_user = element(split("/", data.aws_caller_identity.current.arn), length(split("/", data.aws_caller_identity.current.arn)) - 1)
}

# Single, comprehensive policy containing all required permissions explicitly
resource "aws_iam_policy" "ticket_api_master_policy" {
  name        = "ticket_api_master_management"
...
resource "aws_iam_user_policy_attachment" "attach_policy_to_current_user" {
  user       = local.current_user
  policy_arn = aws_iam_policy.ticket_api_master_policy.arn
}

resource "time_sleep" "wait_for_iam_propagation" {
  depends_on      = [aws_iam_user_policy_attachment.attach_policy_to_current_user]
  create_duration = "40s"
}


resource "aws_iam_policy" "ecs_policy" {
  name        = "ticket_api_ecs_policy"
  description = "IAM policies for ecs service"
...
resource "aws_iam_role" "ecs_task_execution_role" {
  name               = "ecs_ticket_api_execution_role"
